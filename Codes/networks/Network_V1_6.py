...
class AFF1(nn.Module):
    def __init__(self, in_channel, out_channel, ffn_expansion_factor=2, bias=False):
        ...
        self.project_in = nn.Conv2d(in_channel, hidden_features * 2, kernel_size=1, bias=bias)
        self.dwconv = nn.Conv2d(hidden_features * 2, hidden_features * 2, kernel_size=3, stride=1, padding=1,
                                groups=hidden_features * 2, bias=bias)

        self.project_out = nn.Conv2d(hidden_features*2, out_channel, kernel_size=1, bias=bias)
...
class SAM(nn.Module):
    def __init__(self, n_feat, kernel_size=3):
        ...
        self.conv1 = BasicConv(n_feat, n_feat, kernel_size=kernel_size, stride=1, relu=True)
        self.conv2 = BasicConv(n_feat, 3, kernel_size=kernel_size, stride=1, relu=False)
        self.conv3 = BasicConv(3, n_feat, kernel_size=kernel_size, stride=1, relu=False)
...
class MIMOUNet_complete(nn.Module):
    def __init__(self, base_channel=24, num_res=8):
        super(MIMOUNet_complete, self).__init__()
        ...
        self.feat_extract = nn.ModuleList([
            ...
            BasicConv(base_channel * 4, base_channel * 4, kernel_size=3, relu=True, stride=2),
            BasicConv(base_channel*4, base_channel*4, kernel_size=4, relu=True, stride=2, transpose=True),
        ...
        self.Convs = nn.ModuleList([
            BasicConv(base_channel * 4*2, base_channel * 4, kernel_size=1, relu=True, stride=1),
            BasicConv(base_channel * 2*2, base_channel*2, kernel_size=1, relu=True, stride=1),
...
if __name__ == "__main__":
    model = MIMOUNet_complete(base_channel=24, num_res=6)
    print(model)
